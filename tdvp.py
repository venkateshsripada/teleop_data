...
class BatchGenerator:
    ...
    def load_full_data(self):
        ...
        #     print(f"Feature batch shape: {train_features[2].size()}")
        #     train_features[2] is the tensor containing side camera data of shape 32,7,32,32,3
        #     img = train_features[2][0][-1].squeeze()    # First batch, last frame of sequence
...
class FullDataSet:
    ...
    def __getitem__(self, idx):
        ...
        robot_task_data = np.load(train_data_dir + value[0])
        robot_joint_data = np.load(train_data_dir + value[1])

        side_camera_image_names = []
        for image_name in np.load(train_data_dir + value[3]):
            side_camera_image_names.append(np.load(train_data_dir + image_name))

        experiment_number = np.load(train_data_dir + value[4])
        time_steps = np.load(train_data_dir + value[5])
        return [robot_task_data.astype(np.float32), robot_joint_data.astype(np.float32), np.array(side_camera_image_names).astype(np.float32), experiment_number, time_steps]
...
class ACTVP(nn.Module):
    ...
    def forward(self, side_camera, robot_task_data):
        self.batch_size = robot_task_data.shape[1]
        state = robot_task_data[0]
        state.to(device)
        batch_size__ = side_camera.shape[1]
        ...
        outputs = []
        for index, (sample_side_cam, sample_robot_task) in enumerate(zip(side_camera[0:-1].squeeze(), robot_task_data[1:].squeeze())):
            sample_side_cam.to(device)
            sample_robot_task.to(device)
            # 2. Run through lstm:
            if index > context_frames-1:
                hidden_1, cell_1 = self.convlstm1(input_tensor=output, cur_state=[hidden_1, cell_1])
                state_task = torch.cat((state, sample_robot_task), 1)
                robot_and_cam = torch.cat((torch.cat(32*[torch.cat(32*[state_task.unsqueeze(2)], axis=2).unsqueeze(3)], axis=3), hidden_1.squeeze()), 1)
                hidden_2, cell_2 = self.convlstm2(input_tensor=robot_and_cam, cur_state=[hidden_2, cell_2])
                skip_connection_added = torch.cat((hidden_2, output), 1)
                output = self.conv1(skip_connection_added)
                outputs.append(output)
            else:
                hidden_1, cell_1 = self.convlstm1(input_tensor=sample_side_cam, cur_state=[hidden_1, cell_1])
                state_task = torch.cat((state, sample_robot_task), 1)
                robot_and_cam = torch.cat((torch.cat(32*[torch.cat(32*[state_task.unsqueeze(2)], axis=2).unsqueeze(3)], axis=3), hidden_1.squeeze()), 1)
                hidden_2, cell_2 = self.convlstm2(input_tensor=robot_and_cam, cur_state=[hidden_2, cell_2])
                skip_connection_added = torch.cat((hidden_2, sample_side_cam), 1)
                output = self.conv1(skip_connection_added)
                last_output = output
    ...
    def train_full_model(self):
        ...
        for epoch in progress_bar:
            ...
            for index, batch_features in enumerate(self.train_full_loader):
                ...
                # img = side_camera[0][0]    # ith frame batch, 0th batch
                # img = img.cpu().detach().numpy()
                ...
                side_camera_predictions = self.full_model.forward(side_camera=side_camera, robot_task_data=robot_task)  # Step 3. Run our forward pass.

                # Getting the loss groundtruth images and their respective predictions (in this case 2 frames)
                loss = self.criterion(side_camera_predictions, side_camera[context_frames:])
                loss.backward()
                self.optimizer.step()
                ...
                print_loss = self.print_criterion(side_camera_predictions, side_camera[context_frames:])
            ...
            with torch.no_grad():
                for index__, batch_features in enumerate(self.valid_full_loader):
                    ...
                    side_camera_predictions = self.full_model.forward(side_camera=side_camera, robot_task_data=robot_task)
                    val_loss = self.criterion(side_camera_predictions.to(device), side_camera[context_frames:])
                    val_losses += val_loss.item()

                    print_loss = self.print_criterion(side_camera_predictions.to(device), side_camera[context_frames:])
