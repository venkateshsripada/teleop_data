...
model_save_path = "/home/venkatesh/tdvp/teleop_data//models/THDloss_"
train_data_dir = "/home/venkatesh/tdvp/teleop_data//train_out/"
test_data_dir = "/home/venkatesh/tdvp/teleop_data//test_out/"
scaler_dir = "/home/venkatesh/tdvp/teleop_data/scaler_info_universal/"
...
class FullDataSet:
    ...
    def __getitem__(self, idx):
        ...
        return [robot_task_data.astype(np.float32), robot_joint_data.astype(np.float32), np.array(side_camera_image_names), experiment_number, time_steps]
...
class ACTVP(nn.Module):
    ...
    def forward(self, side_camera, robot_joint_data):
        self.batch_size = robot_joint_data.shape[1]
        state = robot_joint_data[0]
        state.to(device)
        ...
        outputs = []
        for index, (sample_side_cam, sample_robot_joint) in enumerate(zip(side_camera[0:-1].squeeze(), robot_joint_data[1:].squeeze())):
            sample_side_cam.to(device)
            sample_robot_joint.to(device)
            # 2. Run through lstm:
            if index > context_frames-1:
                hidden_1, cell_1 = self.convlstm1(input_tensor=output, cur_state=[hidden_1, cell_1])
                state_joint = torch.cat((state, sample_robot_joint), 1)
                robot_and_cam = torch.cat((torch.cat(32*[torch.cat(32*[state_joint.unsqueeze(2)], axis=2).unsqueeze(3)], axis=3), hidden_1.squeeze()), 1)
    ...
    def train_full_model(self):
        ...
        for epoch in progress_bar:
            ...
            for index, batch_features in enumerate(self.train_full_loader):
                ...
                side_camera_predictions = self.full_model.forward(side_camera=side_camera, robot_joint_data=robot_joint)  # Step 3. Run our forward pass.
            ...
            with torch.no_grad():
                for index__, batch_features in enumerate(self.valid_full_loader):
                    ...
                    side_camera_predictions = self.full_model.forward(side_camera=side_camera, robot_joint_data=robot_joint)
